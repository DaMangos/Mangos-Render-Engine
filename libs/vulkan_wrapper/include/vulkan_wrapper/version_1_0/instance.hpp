...
#include <vulkan_wrapper/version_1_0/non_dispatchable_handles.hpp>
#include <vulkan_wrapper/version_1_0/nullhandle.hpp>
...
[[nodiscard]]
std::pair<instance, VkResult const> create_instance(VkInstanceCreateInfo const & info, VkAllocationCallbacks const & allocator) noexcept;

[[nodiscard]]
std::pair<instance, VkResult const> create_instance(VkInstanceCreateInfo const & info) noexcept;
...
class instance
{
  public:
    ...
    [[nodiscard]]
    std::pair<ext::debug_report_callback, VkResult const> create_debug_report_callback(VkDebugReportCallbackCreateInfoEXT const & info) noexcept;

    [[nodiscard]]
    std::pair<ext::debug_utils_messenger, VkResult const> create_debug_utils_messenger(VkDebugUtilsMessengerCreateInfoEXT const & info) noexcept;

    [[nodiscard]]
    std::pair<khr::surface, VkResult const> create_surface(GLFWwindow * const window) noexcept;

    [[nodiscard]]
    std::pair<std::vector<physical_device>, VkResult const> enumerate_physical_devices() noexcept;
...
template <class FunctionPointer>
FunctionPointer vulkan::version_1_0::instance::get_proc_addr(std::string const & name_function)
{
...
template <class FunctionPointer, class... Args>
std::invoke_result_t<FunctionPointer, Args...> vulkan::version_1_0::instance::invoke(std::string const & name_function, Args &&... args)
{
