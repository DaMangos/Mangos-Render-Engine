...
{
  return static_cast<glfw::millimetre>(static_cast<int>(lhs) + static_cast<int>(rhs));
}
...
{
  return static_cast<glfw::millimetre>(-static_cast<int>(value));
}
...
std::basic_istream<CharT, Traits> & operator>>(std::basic_istream<CharT, Traits> & is, glfw::millimetre & value)
{
  ...
  if(is.get() == 'm' and is.get() == 'm')
    value = static_cast<glfw::millimetre>(tmp);
  else
