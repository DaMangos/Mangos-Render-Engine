...
// constexpr std::array const debug_extensions = {VK_EXT_DEBUG_UTILS_EXTENSION_NAME,
//                                                VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME,
//                                                VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME};

constexpr std::array const layers = {"VK_LAYER_KHRONOS_validation"};

vulkan::instance make_instance(std::span<std::string_view const> const)
{
  return vulkan::instance({
    ...
    .enabledLayerCount       = vulkan::ranges::size(layers),
    .ppEnabledLayerNames     = vulkan::ranges::data(layers),
    .enabledExtensionCount   = vulkan::ranges::size(extensions),
    .ppEnabledExtensionNames = vulkan::ranges::data(extensions),
  });
}
