...
vk::UniqueInstance graphics::internal::vulkan_config::create_instance()
{
  VULKAN_HPP_DEFAULT_DISPATCHER.init();

  auto const app_info   = create_application_info();
  auto const extensions = get_required_instance_extensions({.enable_debug = false, .enable_portability = true});
...
vk::UniqueInstance graphics::internal::vulkan_config::create_debug_instance()
{
  VULKAN_HPP_DEFAULT_DISPATCHER.init();

  auto const app_info   = create_application_info();
  auto const extensions = get_required_instance_extensions({.enable_debug = true, .enable_portability = true});
